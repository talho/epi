...
namespace :db do
  namespace :migrate do
    description = "Migrate the database through scripts in epi/db/migrate"
    description << "and update db/schema.rb by invoking db:schema:dump."
    description << "Target specific version with VERSION=x. Turn off output with VERBOSE=false."
    desc description
    task :epi => :environment do
  ...
  namespace :rollback do
    description = "Rollback the database through scripts in epi/db/migrate"
    description << "and update db/schema.rb by invoking db:schema:dump."
    desc description
    task :epi => :environment do
      ActiveRecord::Migration.verbose = ENV["VERBOSE"] ? ENV["VERBOSE"] == "true" : true
  ...
  namespace :seed do
    description = "Add epi seed data to the database"
    desc description
    task :epi => :environment do
